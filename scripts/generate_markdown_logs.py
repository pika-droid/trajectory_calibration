...
def generate_umpire_log(model_name, model_display, csv_path, out_path):
    ...
    lines.append(
        f"- **Evaluation Datasets**: {len(datasets)} Standard Multimodal QA & Hallucination Benchmarks"
    )
    ...
    lines.append("")
    lines.append(
        f"The following table presents the unweighted macro-arithmetic mean of each uncertainty quantification method across all 14 evaluated multimodal benchmarks on **{model_display}**."
    )
    lines.append("")
...
def _format_markdown_comparison_table(
    ...
    for idx, (_, r) in enumerate(df_table.iterrows()):
        ...
        aece_f = (
            format_num(r[adaptive_ece_col], 0, is_percentage=True, decimals=2)
            if pd.notnull(r[adaptive_ece_col])
            else "-"
        )
        ...
        if is_macro:
            table_lines.append(
                f"| {m_disp} | {cat_disp} | {reg_disp} | {ece_f} | {aece_f} | {auc_f} |"
            )
        else:
            stat_disp = f"`{r[status_col]}`" if status_col and status_col in r else "`VALID`"
            table_lines.append(
                f"| {m_disp} | {cat_disp} | {reg_disp} | {ece_f} | {aece_f} | {auc_f} | {stat_disp} |"
            )
...
def _build_vcps_intro_lines(model_display, datasets, bench_path, ump_path):
    lines = []
...
def generate_vcps_vs_baselines_log(model_name, model_display, bench_path, ump_path, out_path):
    ...
    lines = _build_vcps_intro_lines(model_display, datasets, bench_path, ump_path)
    ...
    lines.append("")
    lines.append(
        f"Macro-averaged evaluation metrics of 15 benchmark methods across all 14 datasets for **{model_display}**."
    )
    lines.append("")
    ...
    lines.append(
        f"- **Overall Best ECE (#1 across ALL evaluated methods)**: Our VCPS methods (**VCPS-5D** / **VCPS-17D**) achieve the absolute lowest ECE on **{len(vcps_ece_wins)} out of 14 datasets** ({len(vcps_ece_wins) / 14 * 100:.1f}% win rate)."
    )
    lines.append(
        f"  - **Specific Datasets Won in ECE**: {', '.join([f'`{d}` ({row.best_ece_method}: **{row.best_ece_val:.2f}%**)' for d, row in vcps_ece_wins.set_index('dataset').iterrows()])}."
    )
    lines.append(
        f"- **Win Rate vs. UMPIRE Multi-Pass Baselines in ECE**: VCPS trajectory calibration achieves lower ECE than all four UMPIRE multi-pass baselines on **{14 if model_name == 'm3_llava' else 13} out of 14 datasets**."
    )
    lines.append(
        f"- **Overall Best AUROC (#1 across ALL evaluated methods)**: Our VCPS methods achieve the highest selective prediction AUROC on **{len(vcps_auc_wins)} out of 14 datasets**."
    )
    lines.append(
        f"  - **Specific Datasets Won in AUROC**: {', '.join([f'`{d}` ({row.best_auc_method}: **{row.best_auc_val:.3f}**)' for d, row in vcps_auc_wins.set_index('dataset').iterrows()])}."
    )
    lines.append("")
    ...
    for _, row in win_df.iterrows():
        ...
        lines.append(
            f"| `{row['dataset']}` | {row['best_ece_method']} | **{row['best_ece_val']:.2f}%** | {row['best_auc_method']} | **{row['best_auc_val']:.3f}** | `{tag_str}` |"
        )
    lines.append("")
