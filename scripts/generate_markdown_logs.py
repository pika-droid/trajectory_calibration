...
def generate_umpire_log(model_name, model_display, csv_path, out_path):
    ...
    lines.append("   Computes the spectral dispersion of the normalized sentence embedding covariance matrix $\\mathbf{\\Sigma} = \\frac{1}{K} \\sum_{k=1}^K (\\mathbf{z}_k - \\bar{\\mathbf{z}})(\\mathbf{z}_k - \\bar{\\mathbf{z}})^\\top$ via singular value decomposition (SVD):")
    lines.append("   $$\\mathcal{E}_{\\text{eigen}} = \\frac{\\sum_{i} \\lambda_i^2}{(\\sum_{i} \\lambda_i)^2}$$")
    lines.append("")
    ...
    # Key Observations
    lines.append("### Key Baseline Observations")
    best_auc_row = agg.loc[agg['auc'].idxmax()]
    best_ece_row = agg.loc[agg['cece'].idxmin()]
    best_cpc_row = agg.loc[agg['pearsonr'].idxmax()]
    lines.append(f"- **Top AUROC Method**: `{best_auc_row['method']}` achieves the highest average selective prediction AUROC (**{best_auc_row['auc']:.3f}**).")
    lines.append(f"- **Top Calibration Method**: `{best_ece_row['method']}` attains the lowest post-hoc Calibrated ECE (**{best_ece_row['cece']*100:.2f}%**).")
    lines.append(f"- **Top Correlation Method**: `{best_cpc_row['method']}` demonstrates the strongest linear correlation (**{best_cpc_row['pearsonr']:.3f}**) with model error probability.")
    lines.append(f"- **Computational Cost**: All 4 UMPIRE methods require $K=10$ stochastic rollout passes per sample ($T=0.5$), resulting in an order-of-magnitude increase in generation compute compared to greedy single-pass inference.")
    lines.append("")
...
def generate_vcps_vs_baselines_log(model_name, model_display, bench_path, ump_path, out_path):
    ...
    lines.append("### C. In-Depth Technical Insights & Commentary")
    lines.append("1. **Single-Pass Efficiency vs. Multi-Pass Sampling**:")
    lines.append("   - While UMPIRE baselines (`umpire`, `eigen_score`, `semantic_entropy`, `ln_entropy`) utilize $K=10$ stochastic rollout paths at $T=0.5$, their calibrated ECEs remain significantly elevated (macro mean $17.34\\%$ to $28.25\\%$ on M3-LLaVA, $21.27\\%$ to $33.37\\%$ on MQT-LLaVA).")
    lines.append("   - In contrast, VCPS operates on deterministic greedy decoding ($T=0.0, K=1$) and extracts trajectory-level state features, achieving an order of magnitude lower macro ECE (**3.75%** for VCPS-17D, **4.13%** for VCPS-5D on M3-LLaVA) while requiring **$10\\times$ less computational overhead**.")
    lines.append("")
    lines.append("2. **Why Trajectory Calibration Excels on Structured Visual Reasoning & OCR Tasks**:")
    lines.append("   - On fine-grained OCR, document analysis, and perceptual reasoning tasks (e.g., `docvqa`, `infographicvqa`, `pope`, `mmbench`), autoregressive generation displays distinct trajectory dynamics where early token uncertainty propagates into error cascades.")
    lines.append("   - By capturing token-level entropy trajectories, top-1/top-2 logit margin gaps, hidden-state norms, and attention dispersion across decoding steps, VCPS forms a tight confidence envelope that classic scalar calibrators (TS/Platt) cannot represent.")
    lines.append("")
    lines.append("3. **Macro Trade-Offs**:")
    lines.append("   - **Residual Calibrator** and **VCPS** achieve the strongest macro AUROC scores (~0.69 - 0.70) among single-pass methods, matching or exceeding multi-pass sampling methods without requiring auxiliary DeBERTa NLI cross-encoders or semantic clustering pipelines.")
    lines.append("")
