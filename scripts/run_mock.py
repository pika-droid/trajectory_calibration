...
def main() -> None:
    ...
    print(
        f"Loaded {len(df)} samples. Features: {len(FEATURE_KEYS)} trajectory features (1 anchor + 16 signatures)."
    )
    ...
    # Universal Canonical 5-D Feature Set
    best_5d_keys = CANONICAL_5D_KEYS
    print(f"Selected Canonical 5-D Subset: {best_5d_keys}")
    ...
    print("\n" + "-" * 85)
    print(
        f"{'Method':<28} | {'ECE (%)':<8} | {'Ada-ECE (%)':<11} | {'AUROC':<7} | {'Brier':<7} | {'Status':<9}"
    )
    print("-" * 85)
    ...
    for name, (model, X_tr, X_te) in calibrators.items():
        ...
        print(
            f"{name:<28} | {panel['ece_percent']:<8.2f} | {panel['adaptive_ece_percent']:<11.2f} | {panel['auroc']:<7.3f} | {panel['brier']:<7.4f} | {panel['status']:<9}"
        )
