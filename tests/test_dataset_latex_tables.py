"""
Unit tests for Group 1-4 LaTeX table generation and Option A ranking invariants.
Covers:
1. rank_and_format handles ties and precision collision gracefully.
2. Group 1: 14 per-dataset tables in dataset_tables/dataset_wise_results/ with 8 methods & 4 metrics.
3. Group 2: macro_mean.tex with 8 methods & 4 metrics.
4. Group 3: temp_ablation tables with stacked temperature blocks and 4 methods & 4 metrics.
5. Group 4: lodo_cross_dataset.tex with 4 methods & 4 metrics across both protocols.
6. Strict LOC constraint (< 200 LOC).
"""
...
def test_group1_dataset_wise_tables_exist_and_structure() -> None:
    ...
    for filename in DATASET_FILE_MAP.values():
        ...
        # Check all 8 methods present in both M3 and MQT sections
        for m_key, _, _ in TARGET_METHODS_ORDER:
            # Multi-pass methods use display names
            if m_key == "ln_entropy":
...
def test_group2_macro_mean_table() -> None:
    """Verify Group 2 macro_mean.tex exists and contains all 8 methods and 4 metrics."""
