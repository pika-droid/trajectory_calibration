...
| **VCPS-5D (Our Method)** | $p = \sigma(\exp(a_0 + \gamma^T z_{\text{slope}}) x_1 + b_0 + w^T z_{\text{int}})$ | 10 | Stepwise Visual Trajectory $z \in \mathbb{R}^4$ |
| **VCPS-17D (Our Method)** | $p = \sigma(\exp(a_0 + \gamma^T z_{\text{all}}) x_1 + b_0 + w^T z_{\text{all}})$ | 34 | Full Trajectory Signature $z \in \mathbb{R}^{16}$ |
